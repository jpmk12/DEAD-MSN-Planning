// DoD PKI CA bundle, embedded as a string so it always deploys (some hosts
// strip .pem/.crt files). These are PUBLIC CA certificates — safe to commit.
// Source: user-provided DoD PKE chain (DoD Root CA 6). Add more roots/
// intermediates between the backticks if DAIP chains to a different root.
export const DOD_CA_PEM = `
-----BEGIN CERTIFICATE-----

NKrk3ucpMoHF
-----END CERTIFICATE-----
`;
